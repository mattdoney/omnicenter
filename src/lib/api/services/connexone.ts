...
interface EmailMessage extends UnifiedMessage {
  type: 'email';
  emailAddress: string;
  subject: string;
  isOpened: boolean;
...
interface SMSMessage extends UnifiedMessage {
  type: 'sms';
  phoneNumber: string;
}

interface CallMessage extends UnifiedMessage {
  type: 'call';
  phoneNumber: string;
  duration: number;
  status: string;
