...
export default function Timeline({ messages = [], loading = false }) {
    if (loading) {
    ...
    return (<div className="flex flex-col space-y-6 p-4">
      {groupedMessages.map((group, groupIndex) => (<div key={group.date} className="space-y-4">
          ...
          {group.messages.map((message) => (<div key={message.id} className={`flex ${message.direction === 'outbound' ? 'justify-end' : 'justify-start'}`}>
              <div style={getMessageStyle(message)}>
                <div className="text-sm flex items-center">
                  {getMessageIcon(message.type)}
                  <span>{message.body}</span>
                </div>
                {message.type === 'call' && (<div className="flex flex-col gap-1">
                    {message.userDisplayName && (<div className={`text-xs ${message.direction === 'outbound' ? 'text-gray-200' : 'text-gray-500'}`}>
                        Agent: {message.userDisplayName}
                      </div>)}
                    {message.duration !== undefined && (<div className={`text-xs ${message.direction === 'outbound' ? 'text-gray-200' : 'text-gray-500'}`}>
                        Duration: {Math.floor(message.duration / 60)}:{(message.duration % 60).toString().padStart(2, '0')}
                      </div>)}
                  </div>)}
                {message.status && (<div className={`text-xs mt-1 ${message.direction === 'outbound' ? 'text-gray-200' : 'text-gray-500'}`}>
